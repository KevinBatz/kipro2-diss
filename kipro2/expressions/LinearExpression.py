from pysmt.shortcuts import Times, Symbol, INT, Real, Plus, simplify, ToReal, EqualsOrIff,get_env, Minus,TRUE,FALSE
...
class LinearExpression:
    ...
    def __init__(self, var_to_coeff, abs, is_infty=False):
        self.var_to_coeff = var_to_coeff
        self.abs = abs
        self.is_infty = is_infty

        if not self.is_infty:
    ...
    def add(self, lin_exp):
        ...
        if self.is_infty or lin_exp.is_infty:
            return LinearExpression.get_constant_infty_expression({key for key in self.var_to_coeff})
    ...
    def multiply(self, real_constant):
        ...
        """
        if self.is_infty:
            if real_constant == Real(0):
                return LinearExpression.get_constant_expression({key for key in self.var_to_coeff}, real_constant)
            else:
                return LinearExpression.get_constant_infty_expression({key for key in self.var_to_coeff})
    ...
    @staticmethod
    def get_constant_infty_expression(variables):
        return LinearExpression({var:Real(0) for var in variables}, Real(0), True)
