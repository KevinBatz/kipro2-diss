import logging
from kipro2.solving.Solver import *
...
from kipro2.inequalities.LinearInequality import LinearInequality
...
from kipro2.expectations.CharacteristicFunctional import CharacteristicFunctional
from kipro2.expectations.ErtCharacteristicFunctional import ErtCharacteristicFunctional
...
def parse_program_and_postexp_into_charfun(options):
    ...
    #Will be new
    # Will be old
    execute_guard_prob_sub_tick_triples = get_execute_guard_prob_sub_tick_triples(variables, probably_wp_transformer,
                                                                    probably_summation_nf)

    terminate_guard_linexp_pairs = get_terminate_guard_linexp_pairs(variables, program, probably_summation_nf, options.post)

    char_fun = CharacteristicFunctional(variables, variables_to_bounds, execute_guard_prob_sub_tick_triples, terminate_guard_linexp_pairs)

    # Parse Preexpectation. Must be in gnf
    probably_preexpectation = parse_expectation(options.pre)
    flattened_preexpectation = normalize_expectation_simple(program, probably_preexpectation)
    if type(flattened_preexpectation) == CheckFail:
        raise Exception("CheckFail.")

    preexpectation_guard_linexp_pairs = [
        ...
        probably_guard, probably_linexp in flattened_preexpectation]

    preexpectation_guard_linexp_pairs = merge_pairs_with_equal_guards(preexpectation_guard_linexp_pairs)
    preexpectation = Expectation(variables, preexpectation_guard_linexp_pairs)
    if not preexpectation.check_gnf():
        print(str(preexpectation))
        raise Exception("Candidate upper bound/Preexpectation must be in GNF.")

    return (preexpectation, char_fun)
...
def get_execute_guard_prob_sub_tick_triples(variables, probably_wp_transformer, probably_summation_nf):
    env = get_env()
    ...
    logger.debug("Parsing guard_probsubpairs. Not ignoring ticks.")

    # Probably guards -> lists of linear inequalities
    cegispro_guards = [probably_guard_to_cegispro_guard(variables, probably_guard) for probably_guard in
                       probably_guards]

    # aggregate equals guards and create (guard, [prob_sub_pairs)] list.
    guard_prob_sub_tick_triples = []
    cegispro_guards_copy = cegispro_guards.copy()
    while len(cegispro_guards) > 0:
        cur_guard = cegispro_guards[0]
        cur_prob_sub_tick_list = []
        ...
        for i in range(len(cegispro_guards_copy)):
            ...
            if cur_guard.equals_syntactically(guard):
                ...
                probably_prob = probably_probs[i]
                probably_sub = probably_subs[i]
                probably_tick = probably_ticks[i]
                prob = Real(probably_probability_to_fraction(probably_prob))
                tick = probably_linear_expression_to_cegispro_linear_expression(variables,probably_tick)
                sub = {}
                for var, key in probably_sub.items():
                    sub[env.formula_manager.get_symbol(var)] = probably_linear_expression_to_cegispro_linear_expression(
                        variables, key)
                cur_prob_sub_tick_list.append((prob, sub, tick))
                cegispro_guards.remove(guard)
        guard_prob_sub_tick_triples.append((cur_guard, cur_prob_sub_tick_list))
    ...
    logger.debug("The modified variables are: %s" % str(modC))
    res = get_guard_prob_sub_gnf(guard_prob_sub_tick_triples, 0, [TRUE()], [], [], vars_nat_valued, variables)
    return res


def get_guard_prob_sub_tick_gnf(guard_prob_sub_tick_triples, current_index, current_formula, current_guardindex_truthvalue_list, current_prob_sub_tick_triples, vars_nat_valued, variables):
    if current_index == len(guard_prob_sub_tick_triples):
        if len(current_prob_sub_tick_triples) > 0:
            guard = Guard.TRUE(variables)
            for i in range(len(guard_prob_sub_tick_triples)):
                if current_guardindex_truthvalue_list[i] == 1:
                    guard = Guard.AND(guard, guard_prob_sub_tick_triples[i][0])# if current_guardindex_truthvalue_list[i] == 1 else Guard.AND(guard, Guard.NEG(guard_prob_sub_pairs[i][0]))
            return [(guard.prune_duplicate_disjuncts().prune_duplicate_conjuncts(), current_prob_sub_tick_triples)]
        else:
            return []

    new_formula_true = current_formula + [guard_prob_sub_tick_triples[current_index][0].pysmt_expression]
    new_formula_false = current_formula + [Not(guard_prob_sub_tick_triples[current_index][0].pysmt_expression)]

    res = []
    if is_sat(vars_nat_valued + new_formula_true):
        res = res + get_guard_prob_sub_gnf(guard_prob_sub_tick_triples, current_index + 1, new_formula_true, current_guardindex_truthvalue_list + [1],
                                           current_prob_sub_tick_triples + guard_prob_sub_tick_triples[current_index][1], vars_nat_valued, variables)

    if is_sat(vars_nat_valued + new_formula_false):
        res = res + get_guard_prob_sub_gnf(guard_prob_sub_tick_triples, current_index + 1, new_formula_false, current_guardindex_truthvalue_list + [0],
                                           current_prob_sub_tick_triples, vars_nat_valued, variables)
    return res
...
def probably_linear_expression_to_cegispro_linear_expression(variables, probably_linear_expression):
    ...
    if isinstance(probably_linear_expression, BinopExpr):
        ...
        if probably_linear_expression.is_infinite():
            return LinearExpression.get_constant_infty_expression(variables)
    ...
    elif isinstance(probably_linear_expression, TickExpr):
        if not isinstance(probably_linear_expression.expr, NatLitExpr):
            raise Exception("We allow for integer ticks only.")
        return LinearExpression.get_constant_expression(variables, Real(probably_linear_expression.expr.value))
    else:
        raise Exception("Unexpected expression.: %s" % probably_linear_expression)
