from pysmt.shortcuts import Real, And, GE, Int, LT, Implies, GT, get_env, Or, Equals, Minus, TRUE, FALSE, simplify
...
class Expectation:
    ...
    def pointwise_minimum(exp1, exp2):
        ...
        for (guard1, linexp1) in exp1.guard_linexp_pairs:
             for (guard2, linexp2) in exp2.guard_linexp_pairs:
                ...
                if linexp1.is_infty:
                    if conj.is_sat():
                        new_guard_linexp_pairs.append((conj, linexp2))
                elif linexp2.is_infty:
                    if conj.is_sat():
                        new_guard_linexp_pairs.append((conj, linexp1))
                else:
    ...
    @staticmethod
    def check_leq(exp1, exp2, printmodel = False):
        """
        ...
        #         formula = FALSE() if exp2.guard_linexp_pairs[j][1].is_infty else (And([exp1.guard_linexp_pairs[i][0].pysmt_expression, exp2.guard_linexp_pairs[j][0].pysmt_expression]) if exp1.guard_linexp_pairs[i][1].is_infty and not exp2.guard_linexp_pairs[j][1].is_infty
        #             else
        #             (And([exp1.guard_linexp_pairs[i][0].pysmt_expression, exp2.guard_linexp_pairs[j][0].pysmt_expression,
        ...
        #         if is_sat([formula]): return False
        ...
        formulas = [
            Or([FALSE() if exp2.guard_linexp_pairs[j][1].is_infty else
                (And([exp1.guard_linexp_pairs[i][0].pysmt_expression, exp2.guard_linexp_pairs[j][0].pysmt_expression]) if exp1.guard_linexp_pairs[i][1].is_infty and not exp2.guard_linexp_pairs[j][1].is_infty
                else
                 (And([exp1.guard_linexp_pairs[i][0].pysmt_expression, exp2.guard_linexp_pairs[j][0].pysmt_expression,
                     GT(exp1.guard_linexp_pairs[i][1].pysmt_expression,
                         exp2.guard_linexp_pairs[j][1].pysmt_expression)
        ...
        #print("formulas")
        #print([form.serialize() for form in formulas])
    ...
    def check_non_negativity(self, *additional_constraints):
        ...
        """
        formulas = [self.vars_nat_valued,
                    Or(FALSE() if self.guard_linexp_pairs[i][1].is_infty else And(self.guard_linexp_pairs[i][0].pysmt_expression, LT(self.guard_linexp_pairs[i][1].pysmt_expression, Real(0))) for i in range(len(self.guard_linexp_pairs))
                       )]
