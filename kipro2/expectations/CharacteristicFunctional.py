...
class CharacteristicFunctional():
    ...
    def __init__(self, variables, variables_to_bounds, execute_guard_prob_sub_tick_triples, terminate_guard_linexp_pairs):
        """
        ...
        self.execute_guard_prob_sub_tick_triples = execute_guard_prob_sub_tick_triples
        self.terminate_guards_linexp_pairs = terminate_guard_linexp_pairs

        self.pysmt_execute_guard_prob_sub_tick_triples = [(pair[0].pysmt_expression, pair[1]) for pair in self.execute_guard_prob_sub_tick_triples]
    ...
    def check_all_probs_add_to_one(self):
        ...
        """
        for (_, prob_sub_tick_triples) in self.execute_guard_prob_sub_tick_triples:
            assert (is_sat([Equals(Plus([triple[0] for triple in prob_sub_tick_triples]), Real(1))]))
    ...
    def check_gnf(self):
        ...
        list_of_all_guards = [triples[0] for triples in self.execute_guard_prob_sub_tick_triples] + [pair[0] for pair in self.terminate_guards_linexp_pairs]
        ...
        if not model == None:
            raise Exception("Postexpectation not in GNF." % model)
    ...
    def check_updates_yield_nonnegative_result(self):
        ...
        variables_nat = And([GE(var, Int(0)) for var in self.variables])
        for (pysmt_guard, prob_sub_tick_triples) in self.pysmt_execute_guard_prob_sub_tick_triples:
            for prob_sub_tick_triple in prob_sub_tick_triples:
                sub = prob_sub_tick_triple[1]
    ...
    def apply_expectation(self, exp, prune_unsat_guards_early = True, substitute_helpers_back_to_normals = False, helpvar_to_var = None):
        ...
        for (guard, prob_sub_tick_triples) in self.execute_guard_prob_sub_tick_triples:
            #logger.debug("consider guard %s" % str(guard))
            substituted_exps = [exp.substitute(triple[1]) for triple in prob_sub_tick_triples]
            ...
            while True:
                ...
                #print("vor sat")
                #print([form.serialize() for form in cur_formulas])
                if (not prune_unsat_guards_early) or is_sat(cur_formulas):
                    ...
                    if current_exp == len(index_list) - 1:
                        ...
                        for i in range(len(prob_sub_tick_triples)):
                            prob = prob_sub_tick_triples[i][0]
                            lin_exp = substituted_exps[i].guard_linexp_pairs[index_list[i]][1]
                            #lin_exp = lin_exp.add(prob_sub_tick_triples[i][2])
                            #lin_exp = lin_exp.add(LinearExpression.get_constant_expression(substituted_exps[i].variables, Real(1)))
                            #print(prob_sub_tick_triples[i][2])
                            #print(type(prob_sub_tick_triples[i][2]))
    ...
    def get_list_of_states_satisfying_loop_guard(self):
        ...
        guard_formula = And([guard for (guard, _) in self.pysmt_execute_guard_prob_sub_tick_triples])
        res = []
    ...
    def __str__(self):
        ...
        for (guard, prob_sub_tick_triples) in self.execute_guard_prob_sub_tick_triples:
            res += "[" + str(guard) + "] * ("
            for (prob, sub, tick) in prob_sub_tick_triples:
                res += str(prob) + " * " + "(" + str(tick) +  " + f[" + str({var:str(sub[var]) for var in sub}) + "]) + "
            res = res[:-2]
            res += "\n+"
